...
test.describe("Nutrition Food Search and Logging", () => {
  ...
  test("should open inline search panel when clicking Add on a meal", async ({ page }) => {
    const persona = getPersona("taylor");
  ...
  test("should show search results when typing in search box", async ({ page }) => {
    ...
    const results = layout.locator('[role="option"]');
    await expect(results.first()).toBeVisible({ timeout: 5000 });

    // Verify "chicken" appears in the results
    await expect(layout.locator('[role="listbox"]')).toContainText(/chicken/i);
  });
  ...
  test("should open serving size selector when clicking a search result", async ({ page }) => {
    ...
    await expect(layout.locator("text=/banana/i").first()).toBeVisible();

    // Should have a Confirm button
    await expect(layout.getByRole("button", { name: /confirm/i })).toBeVisible();
  });
  ...
  test("should add food to meal and show in meal section", async ({ page }) => {
    ...
    const confirmButton = layout.getByRole("button", { name: /confirm/i });
  ...
  test("should update macro progress bars after adding food", async ({ page }) => {
    ...
    await expect(layout.getByLabel(/serving size/i)).toBeVisible({ timeout: 3000 });
    await layout.getByRole("button", { name: "50g" }).first().click();
  ...
  test("should handle empty search results gracefully", async ({ page }) => {
    ...
    await expect(layout.locator("text=/no foods found/i")).toBeVisible({ timeout: 3000 });

    // Should also show AI estimate option
    await expect(layout.locator("text=/estimate with ai/i")).toBeVisible();
  });
  ...
  test("should close search panel when clicking Cancel", async ({ page }) => {
    ...
    // Click Cancel button (at the bottom of the search panel)
    const cancelButton = layout.locator("button").filter({ hasText: /cancel/i }).last();
    ...
    await expect(layout.getByRole("button", { name: /add breakfast/i })).toBeVisible();
  });
});
