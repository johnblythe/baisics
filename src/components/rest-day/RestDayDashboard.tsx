...
export function RestDayDashboard({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Recovery Tip Card - Full width, compact */}
      <div
        className="rounded-2xl overflow-hidden"
        style={{
      ...
      {/* Start Workout CTA - for users who want to train anyway */}
      {nextWorkout && (
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              ...
              <p className="text-[#0F172A] font-medium">
                Next up: {nextWorkout.name} ({nextWorkout.focus})
              </p>
            </div>
            <Link
              href={`/workout/${nextWorkout.id}?programId=${programId}`}
              onClick={() => onStartWorkout?.()}
              className="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-[#FF6B6B] to-[#EF5350] text-white font-medium rounded-lg hover:shadow-lg transition-all whitespace-nowrap"
            >
