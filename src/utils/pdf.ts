...
/**
 * Generates a PDF for a workout program
 * @param programId - The ID of the program to generate a PDF for
 * @param userId - The ID of the user requesting the PDF
 */
export const generateWorkoutPDF = async (programId: string): Promise<void> => {
  try {
    // Fetch program data from API
    const response = await fetch(`/api/programs/${programId}/pdf`);
    if (!response.ok) {
      throw new Error('Failed to fetch program data');
    }
    const transformedProgram = await response.json();
    ...
    doc.setFontSize(24);
    doc.setFont('helvetica', 'bold');
    const titleLines = doc.splitTextToSize(transformedProgram.name, doc.internal.pageSize.width - 2 * margin);
    doc.text(titleLines, margin, yOffset);
    yOffset += (titleLines.length * 10) + 5;

    if (transformedProgram.description) {
      doc.setFontSize(12);
      doc.setFont('helvetica', 'normal');
      const descriptionLines = doc.splitTextToSize(transformedProgram.description, doc.internal.pageSize.width - 2 * margin);
      ...
      plan.workouts
        ...
        .forEach((workout) => {
          ...
          workout.exercises.forEach((exercise: Exercise) => {
            ...
            const restPeriodText = formatRestPeriod(exercise.restPeriod);
