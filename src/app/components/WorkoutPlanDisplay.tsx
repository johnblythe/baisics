...
interface ExtendedWorkoutPlan extends WorkoutPlan {
  ...
  bodyCompositionImages?: UploadedImage[];
  name?: string;
  description?: string;
}
...
export function WorkoutPlanDisplay({ userEmail: initialUserEmail, plan, onRequestUpsell, onUploadImages, onDeleteImage }: WorkoutPlanDisplayProps) {
          ...
          <div className="flex justify-between items-start">
            <div className="space-y-2">
              <h1 className="text-4xl font-bold text-white">
                {plan.name || "Flibb flubb"}
              </h1>
              <p className="text-lg text-indigo-100">
                {plan.description || "awelfkj fjekwfjw ef alwkefj alwef awelkfjaw elfkj awelfkj awelfk jawef"}
              </p>
            </div>
              ...
              true && (
                <div className="space-y-3">
                  <div className="flex justify-between items-center p-3 bg-white dark:bg-gray-700 rounded-lg transition-all duration-300 hover:shadow-md hover:-translate-y-0.5">
                    <span className="text-gray-600 dark:text-gray-300">Body Fatz:</span>
                  ...
                  <div className="mt-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {plan.bodyCompositionImages?.map((image) => (
                        <div 
                          key={image.id} 
              ...
              >
                ...
                >
                  ...
                  <div className="absolute inset-y-0 left-0 right-0 flex items-center justify-between px-4">
                    {plan.bodyCompositionImages && plan.bodyCompositionImages.findIndex(img => img.id === selectedImage.id) > 0 && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const currentIndex = plan.bodyCompositionImages!.findIndex(img => img.id === selectedImage.id);
                          setSelectedImage(plan.bodyCompositionImages![currentIndex - 1]);
                        }}
                    ...
                    {plan.bodyCompositionImages && plan.bodyCompositionImages.findIndex(img => img.id === selectedImage.id) < plan.bodyCompositionImages.length - 1 && (
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          const currentIndex = plan.bodyCompositionImages!.findIndex(img => img.id === selectedImage.id);
